...
@endif
    <a href="/Tambahdata" type ="button" class="btn btn-outline-danger">Tambah Data</a><br><br>
        <div class="row">
...
@foreach ($data as $row)
   <tr>
      ...
      <td>{{ $row->jurusan }}</td>
      <td>
         <img alt="" src="logosekolah/{{$row->logo }}" style="width:70px;height:100px;">
      </td>
      <td>
        <a href="tampilkandata/ {{ $row->id }}" class="btn btn-outline-info">Ubah</a>
            <a href="/delete/ {{ $row->id }}"class="btn btn-outline-danger" data-id="{{ $row->id }}"
            <?php echo $row['id']; ?>
